      ...
      <div v-else>
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          <h2 class="text-xl font-semibold text-gray-800">
            {{ plantsCount }} plantes dans cette catégorie
          </h2>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.plants = plantsResponse.data.filter(
        (plant) => plant.category_id === this.category.id || plant.category === this.category.name,
      )
    } catch (err) {
      this.error =
