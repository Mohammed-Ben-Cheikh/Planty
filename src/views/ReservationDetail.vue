<template>
    <main class="flex-grow container mx-auto px-4 sm:px-6 py-8 max-w-7xl">
        <div class="animate-fade-in">
                    ...
                    <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-6">
                        <div>
                            ...
                            <div class="mt-2">
                                ...
                                <p v-if="order.user?.phone" class="text-sm text-gray-900">{{ order.user?.phone }}</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                ...
                                <p v-if="order.city" class="text-sm text-gray-900">{{ order.postal_code }} {{ order.city
                                    }}</p>
                                    ...
                                    <div class="ml-4 flex-1 flex flex-col">
                                        <div>
                                            <div class="flex justify-between text-base font-medium text-gray-900">
                                                <h4>{{ item.name }}</h4>
                                                <p class="ml-4">{{ (item.quantity * parseFloat(item.price.replace(',',
                                                    '.'))).toFixed(2) }}€</p>
                                            </div>
                                            <p v-if="item.species" class="mt-1 text-sm text-gray-500">{{ item.species }}
                                            </p>
                                        </div>
            ...
            <div v-if="order && order.history && order.history.length > 0"
    ...
    methods: {
        async loadOrder() {
            ...
            try {
                ...
                this.error = err.response?.data?.message || 'Une erreur est survenue lors du chargement de la réservation.';
            } finally {
